const fs = require('fs');
import {parser} from './';
var parser = require('./src/assets/JS/gramatica.js');


// Constantes para operaciones, instrucciones y valores ./src/Interprete/instrucciones.js
import { TIPO_INSTRUCCION } from './instrucciones';
//const TIPO_INSTRUCCION = require('./src/assets/JS/instrucciones.js').TIPO_INSTRUCCION;
const TIPO_OPERACION = require('./instrucciones').TIPO_OPERACION;
const TIPO_VALOR = require('./instrucciones').TIPO_VALOR;
const instruccionesAPI = require('./instrucciones').instruccionesAPI;
const TIPO_OPCION_SWITCH = require('./instrucciones').TIPO_OPCION_SWITCH

// Tabla de Simbolos ./src/Interprete/tabla_simbolos.js
const TIPO_DATO = require('./src/Interprete/tabla_simbolos.js').TIPO_DATO;
const TS = require('./src/assets/JS/tabla_simbolos.js').TS;
...
try {
    ...
    const entrada = fs.readFileSync('./src/assets/JS/entrada.txt');
    // invocamos a nuestro parser con el contendio del archivo de entradas
    ast = parser(entrada.toString());

    // imrimimos en un archivo el contendio del AST en formato JSON
    fs.writeFileSync('./src/assets/JS/ast.json', JSON.stringify(ast, null, 2));
} catch (e) {
    console.error(e);
 ...
 */
function procesarBloque(instrucciones: any[], tablaDeSimbolos: any) {
 ...
 */
function procesarExpresionNumerica(expresion:any, tablaDeSimbolos:any) {
    if (expresion.tipo === TIPO_OPERACION.NEGATIVO) {
        ...
        const valor:any = procesarExpresionNumerica(expresion.operandoIzq, tablaDeSimbolos).valor; // resolvemos el operando
        ...
        let valorIzq:any = procesarExpresionNumerica(expresion.operandoIzq, tablaDeSimbolos); // resolvemos el operando izquierdo.
        let valorDer:any = procesarExpresionNumerica(expresion.operandoDer, tablaDeSimbolos); // resolvemos el operando derecho.
 ...
 */
function procesarExpresionCadena(expresion:any, tablaDeSimbolos:any) {
    if (expresion.tipo === TIPO_OPERACION.CONCATENACION) {
        ...
        const cadIzq:any = procesarExpresionCadena(expresion.operandoIzq, tablaDeSimbolos).valor; // resolvemos el operando izquierdo.
        const cadDer:any = procesarExpresionCadena(expresion.operandoDer, tablaDeSimbolos).valor; // resolvemos el operando derecho.
 ...
 */
function procesarExpresionRelacional(expresion:any, tablaDeSimbolos:any) {
 ...
 * Evaluamos cada caso para resolver a un valor tipo booleando de acuerdo al tipo de operación.
 */
function procesarExpresionLogica(expresion:any, tablaDeSimbolos:any) {
 ...
 * @param {*} tablaDeSimbolos 
 */
function procesarImprimir(instruccion:any, tablaDeSimbolos:any) {
    const cadena:string = procesarExpresionCadena(instruccion.expresionCadena, tablaDeSimbolos).valor;
    console.log('> ' + cadena);
}
 ...
 * @param {*} tablaDeSimbolos 
 */
function procesarDeclaracion(instruccion:any, tablaDeSimbolos:any) { //aqui cambiamos para que acepte el tipo_dato de la declaracion
 ...
 * @param {*} tablaDeSimbolos 
 */
function procesarAsignacion(instruccion:any, tablaDeSimbolos:any) {
 ...
 */
function procesarMientras(instruccion:any, tablaDeSimbolos:any) {
 ...
 */
function procesarPara(instruccion:any, tablaDeSimbolos:any) {
    ...
    for (var i = tablaDeSimbolos.obtener(instruccion.variable); procesarExpresionLogica(instruccion.expresionLogica, tablaDeSimbolos); tablaDeSimbolos.actualizar(instruccion.variable, { valor: tablaDeSimbolos.obtener(instruccion.variable).valor + 1, tipo: TIPO_DATO.NUMERO })) {
 ...
 */
function procesarIf(instruccion:any, tablaDeSimbolos:any) {
 ...
 * @param {*} tablaDeSimbolos 
 */
function procesarIfElse(instruccion:any, tablaDeSimbolos:any) {
    const valorCondicion = procesarExpresionLogica(instruccion.expresionLogica, tablaDeSimbolos);
 ...
 * @param {*} tablaDeSimbolos 
 */
function procesarSwitch(instruccion:any, tablaDeSimbolos:any) {
    var evaluar = true;
    ...
    instruccion.casos.forEach((caso: { tipo: any; expresionNumerica: any; instrucciones: any; }) => {
 ...
 * @param {*} tablaDeSimbolos 
 */
function procesarAsignacionSimplificada(instruccion:any, tablaDeSimbolos:any) {
