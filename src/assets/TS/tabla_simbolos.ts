 ...
 * @param {*} valor 
 */
function crearSimbolo(id: any, tipo: any, valor: any) {
    return {
        id: id,
        tipo: tipo,
        valor: valor
...
class TS {
     ...
     * @param {*} valor 
     */
    actualizar(id: any, valor: any) { //AQUI VAMOS A VALIDAR TIPOS
        const simbolo = this._simbolos.filter((simbolo: { id: any; }) => simbolo.id === id)[0];
        if (simbolo) {
